...
def _resolve_git_root(candidate_dir: str = None) -> str:
    """
    개발 저장소의 Git 루트 디렉토리를 탐색하여 반환합니다.
    환경변수, 설정 파일, CWD, 표준 개발 디렉토리 순서로 탐색합니다.
    """
    candidates = []
    if candidate_dir:
        candidates.append(candidate_dir)
    
    env_dev_dir = os.getenv("TIERBRIDGE_DEV_DIR")
    if env_dev_dir:
        candidates.append(env_dev_dir)

    source_path_file = os.path.expanduser("~/.tierbridge/source_repo.path")
    ...
    candidates.append(os.path.expanduser("~/Documents/agent-cli"))
        ...
        if c and os.path.exists(c):
            try:
                ...
                if root_check.returncode == 0 and root_check.stdout.strip():
                    return root_check.stdout.strip()
            except Exception:
                continue
    return None
...
def get_git_sync_status(force: bool = False, repo_dir: str = None) -> dict:
    ...
    try:
        git_root = _resolve_git_root(repo_dir)
        if not git_root:
        ...
        if branch and branch != "HEAD":
            ...
            behind_check = subprocess.run(
                ["git", "rev-list", f"HEAD..{target_ref}", "--count"],
                cwd=git_root,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
            ...
            if behind_check.returncode == 0:
                ...
                if behind_count > 0:
                    ...
                    log_check = subprocess.run(
                        ["git", "log", "-n", "5", f"HEAD..{target_ref}", "--oneline"],
                        cwd=git_root,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE,
                        text=True,
            ...
            ahead_check = subprocess.run(
                ["git", "rev-list", f"{target_ref}..HEAD", "--count"],
                cwd=git_root,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
