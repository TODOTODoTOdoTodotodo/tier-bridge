...
2. **기억저장소 탭 컴포넌트**:
   * **KPI 카드 3종**: 누적 지식 에피소드, 활성 도메인 태그, 코드 수정 지식 비율
   * **연관 기억 실시간 검색기**: 검색어 입력 시 `GET /v1/dashboard/memories/search`를 비동기 호출하여 상위 연관 지식 카드를 실시간 렌더링.
   * **실시간 기억 스트림 테이블**: 상단 세션 드롭다운 필터와 양방향 연동되어 특정 세션의 기억만 즉시 필터링.
...
2. **대시보드 실시간 렌더링 검증**: `./analyze_usage.py --html --no-open` 실행 후 탭 전환 및 검색 기능 정상 동작 확인.
