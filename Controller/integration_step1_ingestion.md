...
- **문제-해결 에피소드 번들링 & 원본 질문 고정 정책 (Question-Solution Pairing)**:
  - 내부 서브스텝(`[Substep]`) 릴레이 중이라도 기억저장소의 [문제(Problem)] 필드는 에이전트의 독백이 아닌 **사용자의 실제 원본 질문(`user_prompt`)**을 고정 보존.
  - 해당 질문에 대해 LLM이 생성한 최종 해결책/수정 코드(`solution_text`)와 1개의 완전한 [질문 ➔ 해결책] 지식 에피소드로 결합 적재.
